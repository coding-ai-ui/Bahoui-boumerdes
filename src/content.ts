...
export function renderContent() {
 ...
 <section id="contact" class="contact section-wrap"><div class="section-label"><span class="eyebrow">05 — RETROUVEZ-NOUS</span><span class="fine-line"></span></div><div class="contact-layout"><div><h2>On se retrouve<br><em>à Boumerdès.</em></h2><div class="address"><h3>Rahoui Boumerdes</h3><p>Boumerdès, Algérie</p></div><a href="tel:${b.phone}" class="contact-call" aria-label="Appeler Rahoui au ${b.phoneDisplay}">${b.phoneDisplay}</a><div class="contact-buttons"><a class="button button-dark" href="tel:${b.phone}">Appeler Rahoui <span aria-hidden="true">↗</span></a>${external(b.maps,'Itinéraire ↗','button')}</div><p class="contact-note">Une question ou une table à réserver ?<br>Appelez directement notre équipe.</p></div><figure class="contact-photo">${image('facade.jpg','Entrée du restaurant et de la pâtisserie Rahoui Boumerdès')}${external(b.maps,'Voir sur Google Maps ↗','photo-location')}</figure></div></section>
 <footer><div class="footer-main"><a href="#accueil" class="footer-brand" aria-label="Rahoui, retour à l’accueil"><img src="/images/rahoui-logo.jpg" alt="Rahoui — Pâtisserie Restaurant" width="150" height="150" loading="lazy" /></a><div class="footer-identity"><p class="footer-name">RAHOUI</p><p>Café · Pâtisserie · Restaurant</p><p class="eyebrow">BOUMERDÈS — ALGÉRIE</p></div><nav aria-label="Navigation de pied de page"><a href="#maison">La Maison</a><a href="#carte">La Carte</a><a href="#patisserie">Pâtisserie</a><a href="#galerie">Galerie</a><a href="#contact">Contact</a></nav><nav aria-label="Réseaux et adresse"><a href="tel:${b.phone}">${b.phoneDisplay}</a>${external(b.instagram,'Instagram ↗')}${external(b.tiktok,'TikTok ↗')}${external(b.maps,'Google Maps ↗')}</nav></div><div class="footer-bottom"><span>© 2026 Rahoui Boumerdes</span><a href="#accueil">Retour en haut ↑</a></div></footer>
 <nav class="mobile-actions" aria-label="Accès rapide"><a href="tel:${b.phone}" aria-label="Appeler Rahoui">Appeler</a>${external(b.maps,'Itinéraire ↗')}<button type="button" data-reserve>Réserver</button></nav>
